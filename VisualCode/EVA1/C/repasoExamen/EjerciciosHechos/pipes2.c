	...
	//eso es lo que se utiliza para que si salgan numeros aleatorios(?) (srand==math.Random)
        srand(time(NULL));
	   	...
	   	//lee hazta que el padre envíe un 0 el primer hijo lee lo que recibe del padre
		   	while(0!=read(tuberia1[LECTURA],&entero, sizeof(entero))){
			 	...
			 	//AQUI LO MISMO, EL PADRE PASA NUMEROS Y SI ES 0 TERMINA, SI NO ES MULTIPLO DE 4 LOS VA SUMANDO
			      	while(0!=read(tuberia2[0], &entero, sizeof(entero))){
						...
						if(entero%4!=0){
							...
							suma2=suma2+entero;
							printf("SOY HIJO 2 Y TENGO NUMERO %d",numero);
						} if (entero==0){
							// OPCIONAL close(tuberia2[LEE]);
							write(tuberia2[ESCRITURA], &suma2, sizeof(suma2));	
							exit(0);
						}
			      	}
			}
			...
			for(int i=0;i<10;i++){
				...
				if(entero%4==0){
					write(tuberia1[ESCRITURA], &entero, sizeof(entero)); //escribo en pipe DE MULTIPLES DE 4
					printf("El PADRE ENVIA MENSAJE AL HIJO multiplos ...%d\n", entero);

				}else{
					write(tuberia2[ESCRITURA], &entero, sizeof(entero)); //escribo en pipe DE MULTIPLES !=4
			...
			write(tuberia1[ESCRITURA], &fin, sizeof(fin));
			write(tuberia2[ESCRITURA], &fin, sizeof(fin));
			...
			read(tuberia1[LECTURA], &entero, sizeof(entero));
			read(tuberia2[LECTURA], &entero2, sizeof(entero2));
